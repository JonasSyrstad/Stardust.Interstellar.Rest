...
namespace Stardust.Interstellar.Rest.Test
{
    ...
    public class CustomXmlSerializer : ISerializer
    {
        ...
        public void Serialize(WebRequest req, object val)
        {
            if (val == null) return;
            var serializer = new XmlSerializer(val.GetType());
            using (var writer = new XmlTextWriter(req.GetRequestStream(), Encoding.UTF8))
            {
        ...
        public object Deserialize(Stream responseStream, Type type)
        {
            using (var sr = new StreamReader(responseStream))
            {
                ...
                if (!data.StartsWith("<?mxl")) data = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>" + data;
                var xmlDoc = new XmlDocument();
                xmlDoc.LoadXml(data);
                //var s = xmlDoc.InnerText;
                //Logging.DebugMessage(s);
                
                //var json = JsonConvert.SerializeXmlNode(xmlDoc);
                var json = XmlToJSON(xmlDoc);
                Logging.DebugMessage(json);
